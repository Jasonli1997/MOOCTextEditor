	...
	private void processText()
	{
		...
		for (int i = 0; i < tokens.size(); i++) {
			if (isWord(tokens.get(i))) {
				numWords++;
				int numOfSyllables = countSyllables(tokens.get(i));
				numSyllables += numOfSyllables;				
			} else {
				numSentences++;
			}
			
			// Check for the special case where the sentence does not end with a punctuation
			// isWord method is pretty handy
			if (i == tokens.size() - 1) {
				if (isWord(tokens.get(i))) {
					numSentences+=1;
				}
